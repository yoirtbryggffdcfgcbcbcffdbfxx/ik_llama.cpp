...
void server_context::init() {
    ...
    // LFM2/LFM2.5 templates are model-specific Jinja templates and have no
    // equivalent in the legacy llama_chat_apply_template switch.  Without
    // this, the server can start successfully but the OpenAI/UI path renders
    // the prompt with the wrong format and has no PEG parser for <think>.
    // Enable Jinja for this architecture so the model metadata template and
    // its reasoning/tool parser are used even when --jinja was omitted.
