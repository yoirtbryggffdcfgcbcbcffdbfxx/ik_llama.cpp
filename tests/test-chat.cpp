...
static void test_template_output_peg_parsers(bool detailed_debug) {
    ...
    // LFM2.5 tests - the official template starts generation after <think> and
    // wraps tool calls in <|tool_call_start|>...<|tool_call_end|>.  The parser
    // also retains compatibility with the older bare [name(args)] form.
    {
        auto tst = peg_tester("models/templates/LFM2.5-Instruct.jinja", detailed_debug);

        // The official prompt pre-fills <think>, so a response with no
        // reasoning still starts with the closing tag.
        ...
        // The official template already opens <think> in the generation prompt.
        // Extraction must work even when enable_thinking was not supplied by a
        // client using the template's preserve_thinking convention.
        tst.test("I'm\nthinking</think><|tool_call_start|>[special_function(arg1=1)]<|tool_call_end|>")
            .enable_thinking(false)
            .reasoning_format(COMMON_REASONING_FORMAT_AUTO)
            .tools({ special_function_tool })
