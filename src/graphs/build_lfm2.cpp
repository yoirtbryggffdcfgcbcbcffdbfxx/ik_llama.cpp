...
// Build one LFM2 short-convolution block.  The local ggml SSM_CONV operator
// consumes the rolling state separately from the input matrix.  LFM2 stores
// only the last l_cache - 1 input vectors in the qnext state cache.
static ggml_tensor * build_lfm2_shortconv(
    ...
    // Select the state row for this sequence.  A reset is a graph operation so
    // that it is ordered before SSM_CONV and does not mutate the cache on the CPU.
    ...
    // {n_embd, n_tokens} -> {3*n_embd, n_tokens} and split B, C and X.
    ...
    // The short-convolution gate is elementwise before the causal convolution.
    ...
    // Persist columns 1..d_conv-1 of the new state.  The view is strided by
    // d_conv because the operator appends the current input as column 0.
...
ggml_cgraph * llm_build_context::build_lfm2() {
    ...
    // The local SSM_CONV API always uses a one-slot sequence map for the
    // selected state row.  The actual row is selected from s_l below.
    ...
    if (batch.n_seq_id != nullptr && batch.seq_id != nullptr) {
        for (int64_t i = 0; i < n_tokens; ++i) {
            ...
            // LFM2 currently follows the same one-sequence-per-token contract
            // as the qnext hybrid path.
    ...
    for (int il = 0; il < n_layer; ++il) {
        ...
        if (hparams.is_recurrent(il)) {
            ...
            if (all_same) {
                ...
                // Independent sequence states can be evaluated one token at a
                // time.  The caller already falls back to single-token chunks
                // when a sequence is repeated in a mixed batch.
                op_out = nullptr;
                for (int64_t i = 0; i < n_tokens; ++i) {
            ...
            // The shared operator norm above is also the input to GQA.  Passing
            // a null attention norm avoids applying it twice.
    ...
    // LFM2's embedding norm is the final norm and the LM head is tied to the
    // token embeddings (there is no separate output_norm tensor).
