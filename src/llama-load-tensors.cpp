...
bool create_tensors_helper::create_lfm2_tensors(const LLM_TN & tn) {
    ...
    // LFM2 has one final embedding RMSNorm and a tied LM head.  The HF tensor
    // is called embedding_norm and is mapped to token_embd_norm in GGUF.
    model.tok_embd = create_tensor(ctx_input, tn(LLM_TENSOR_TOKEN_EMBD, "weight"), {n_embd, n_vocab});
    model.tok_norm = create_tensor(ctx_output, tn(LLM_TENSOR_TOKEN_EMBD_NORM, "weight"), {n_embd});
    ...
    for (int i = 0; i < n_layer; ++i) {
        ...
        if (hparams.is_recurrent(i)) {
            // Reuse the existing recurrent tensor slots for LFM2 shortconv.
            // The names remain architecture-specific in the GGUF mapping.
        ...
        for (int il = 0; il < n_layer; ++il) {
            ...
            if (hparams.is_recurrent(il)) {
                if (model.arch == LLM_ARCH_LFM2) {
                    // LFM2 shortconv has no delta-net heads. Its three matrices
                    // are kept whole and execute on the layer's matrix backend.
