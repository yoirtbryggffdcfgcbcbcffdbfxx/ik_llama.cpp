...
class LFM2MoeModel(LFM2Model):
    ...
    Same hybrid shortconv/GQA blocks as the dense models; the first
    ``num_dense_layers`` blocks keep a SwiGLU FFN, all the following ones use a
    sigmoid-routed mixture of experts with a routing bias (``expert_bias``).
    """
    ...
    def set_gguf_parameters(self):
        # the base converter writes the expert count from num_local_experts; LFM2-MoE
        # configs call it num_experts, so hide it here and write the count below
    ...
    def modify_tensors(self, data_torch: Tensor, name: str, bid: int | None) -> Iterable[tuple[str, Tensor]]:
        ...
        # transformers >= 5 fuses the routed experts into two 3D parameters that also
        # carry no suffix: gate_up_proj is [n_expert, 2 * moe_intermediate_size, n_embd]
        # with the gate half first (the Lfm2MoeExperts forward chunks it into gate/up),
        # down_proj is [n_expert, n_embd, moe_intermediate_size]. Split them so the GGUF
        # matches the layout the older per-expert checkpoints are converted to.
